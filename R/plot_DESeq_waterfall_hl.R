...
#' Plot DESeq waterfall with genes highlighted
#'
#' Revamp of Favour's original function
#'
#' @param sig df; DESeq signature with "sign_log_p" and "gene" columns
...
plot_DESeq_waterfall_hl <- function(sig, sig_hl, topn = 200, rev = F, toplabel = "Top SCN",
                                    title = "DE Genes", subtitle = NA,
                                    save = F, savename = "DESeq_slogp_WF_hl.png") {

  sig <- sig[order(sig$sign_log_p, decreasing = T),]
  sig$rank <- nrow(sig):1
  ...
  a = ggbarplot(
    ...
    y = "sign_log_p",
    xlab = "Gene Rank",
    ylab = "DESeq Signed log p-values",
    title = title,
    rotate = T,
    palette = colors,
    fill = "type",
  ...
  # Figure out dimensions and scale
  ...
  a <- a +
    ...
    geom_text(x = xpos_top, y = ypos, label = "Post-Therapy") +
    geom_text(x = xpos_mid, y = ypos, label = paste0("KS enrich. p-value = ", signif(ks_pval, digits = 4))) +
    geom_text(x = xpos_bot, y = ypos, label = "Pre-Therapy") +
