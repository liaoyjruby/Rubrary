...
utils::globalVariables(c("Type", "Batch",
...
run_DESeq <- function(mtx_rawcts, mtx_annotation, outname, merged = F, shrink = F){
  ...
  # Paired
  dds_simple <- DESeq2::DESeqDataSetFromMatrix(countData = mtx,
                                       colData = anno,
...
plot_DESeq_scatter <- function(sig1path, sig2path, batch = "",
  ...
  plt <- ggplot2::ggplot(sig_merged, ggplot2::aes_string(x = "sign_log_p.1", y = "sign_log_p.2")) +
    ...
    ggplot2::theme_bw() +
    ggplot2::labs(title = title, # include comp % on axis
         subtitle = desc,
         x = xlab,
         y = ylab)
  print(plt)
...
plot_DESeq_boxplot <- function(sig, data, anno, batch, merged = T){
  ...
  ggplot2::ggsave(filename = paste0(dir.db, "/OV/DESeq/", title, "_Pos_", batch,".png"),
  ...
  ggplot2::ggsave(filename = paste0(dir.db, "/OV/DESeq/", title, "_Neg_", batch,".png"),
