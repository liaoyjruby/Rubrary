...
#' @param plot_method string; make heatmap with ggplot geom_raster or lattice levelplot
...
#' @param metricsct logical; T to output metric scatterplot
#' @param ranksct logical; T to output rank scatterplot
#'
#' @return RRHO results
#' @export
...
run_RRHO <- function(sig1, sig2, sig1_name, sig2_name,
                     ...
                     plot_method = c("ggplot", "lattice"), palette = "Spectral",
                     waterfall = T, metricsct = T, ranksct = T){
  ...
  if(webtool){
    ...
    message(paste0("Step size (default): ", steps))
  }
  ...
  if(plot_method == "lattice"){ # lattice style heatmap
    requireNamespace("lattice", quietly = TRUE)
    plt <- lattice::levelplot(obj_RRHO$hypermat,
                              xlab = '',
                              ylab = '',
    ...
    plt_hypmtx <- ggplot(hypmtx_long, aes(x = row, y = col, fill = value)) +
      ...
      annotate("text", x = 0, y = -5, label = sig1_high, hjust = "left") +
      annotate("text", x = nrow(hypmtx), y = -5, label = sig1_low, hjust = "right") +
      # y-axis / sig2 annotations
      annotate("text", x = -5, y = 0, label = sig2_high, hjust = "left", angle = 90) +
      annotate("text", x = -5, y = nrow(hypmtx), label = sig2_low, hjust = "right", angle = 90) +
